...
Starting with a set of rows from one or more tables, it respects foreign key constraints
by recursively fetching the "parents" and (optionally) "children" of those rows.
...
_Non-intrusive_: connections to the origin database are read-only. Connections to the
                 target database only require [DML](https://stackoverflow.com/a/2578207)
                 write permission.
