...
In this case, either the porting over of roles to the "target" database must be done manually, or `pg_dump` can be run with the `--no-privileges` and `--no-owner` options so that missing roles are ignored when dumping out `pre-data-dump.sql`.
