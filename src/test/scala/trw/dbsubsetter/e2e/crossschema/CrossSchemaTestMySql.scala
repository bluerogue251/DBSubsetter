...
import trw.dbsubsetter.e2e.{AbstractMysqlEndToEndTest, MysqlEndToEndTestUtil}
...
class CrossSchemaTestMySql extends AbstractMysqlEndToEndTest with CrossSchemaTest {
  ...
  override def prepareTargetDDL(): Unit = {
    ...
    s"./src/test/trw.dbsubsetter.util/sync_mysql_origin_to_target.sh ${containers.origin.name} schema_1 ${containers.targetSingleThreaded.name} schema_1 ".!!
    s"./src/test/trw.dbsubsetter.util/sync_mysql_origin_to_target.sh ${containers.origin.name} schema_2 ${containers.targetSingleThreaded.name} schema_2 ".!!
    s"./src/test/trw.dbsubsetter.util/sync_mysql_origin_to_target.sh ${containers.origin.name} schema_3 ${containers.targetSingleThreaded.name} schema_3 ".!!
    ...
    s"./src/test/trw.dbsubsetter.util/sync_mysql_origin_to_target.sh ${containers.origin.name} schema_1 ${containers.targetAkkaStreams.name} schema_1 ".!!
    s"./src/test/trw.dbsubsetter.util/sync_mysql_origin_to_target.sh ${containers.origin.name} schema_2 ${containers.targetAkkaStreams.name} schema_2 ".!!
    s"./src/test/trw.dbsubsetter.util/sync_mysql_origin_to_target.sh ${containers.origin.name} schema_3 ${containers.targetAkkaStreams.name} schema_3 ".!!
  }
}
