...
abstract class AbstractPostgresqlEndToEndTest extends AbstractEndToEndTest[PostgreSQLDatabase] {
  ...
  override protected def postSubset(): Unit = {
    s"./src/test/trw.dbsubsetter.util/postgres_post_subset.sh ${containers.origin.name} ${containers.origin.db.name} ${containers.targetSingleThreaded.name} ${containers.targetSingleThreaded.db.name}".!!
    s"./src/test/trw.dbsubsetter.util/postgres_post_subset.sh ${containers.origin.name} ${containers.origin.db.name} ${containers.targetAkkaStreams.name} ${containers.targetAkkaStreams.db.name}".!!
  }
}
...
object PostgresqlEndToEndTestUtil {
  ...
  def syncDDL(originContainer: String, originDb: String, targetContainer: String, targetDb: String): Unit = {
    s"./src/test/trw.dbsubsetter.util/sync_postgres_origin_to_target.sh $originContainer $originDb $targetContainer $targetDb".!!
  }
}
