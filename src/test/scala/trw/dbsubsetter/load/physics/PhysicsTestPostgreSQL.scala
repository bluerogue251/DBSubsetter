package trw.dbsubsetter.load.physics
...
class PhysicsTestPostgreSQL extends AbstractPostgresqlEndToEndTest with LoadTest[PostgreSQLDatabase] with PhysicsTest {
  ...
  override protected def prepareOriginDDL(): Unit = {
    (mustReCreateOriginDb, skipOriginDbPerformanceOptimization) match {
      ...
      case (true, false) => // Load origin DB from dump file stored in S3
        val dumpUrl = "https://s3.amazonaws.com/db-subsetter/trw.dbsubsetter.load-test/physics-db/pgdump.sql.gz"
  ...
  override protected def prepareTargetDDL(): Unit = {
     ...
     */
    s"./src/test/scala/trw.dbsubsetter.load/physics/copy_domain_data_postgres.sh ${containers.origin.name} ${containers.origin.db.name} ${containers.targetSingleThreaded.name} ${containers.targetSingleThreaded.db.name}".!!
    s"./src/test/scala/trw.dbsubsetter.load/physics/copy_domain_data_postgres.sh ${containers.origin.name} ${containers.origin.db.name} ${containers.targetAkkaStreams.name} ${containers.targetAkkaStreams.db.name}".!!
  }
