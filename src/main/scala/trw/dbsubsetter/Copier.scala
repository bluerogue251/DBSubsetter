...
object Copier {
  def copyToTargetDB(originConn: Connection, targetConn: Connection, pkDefinition: PrimaryKey, pkValues: Set[Vector[AnyRef]]): Unit = {
    ...
    val copyOutStatement: SqlQuery =
      s"""copy (select *
         ...
         |      in (
         |        ${pkValues.map(pk => pk.mkString("(", ",", ")")).mkString(",\n")}
         |      )
         |to stdout
       """.stripMargin
